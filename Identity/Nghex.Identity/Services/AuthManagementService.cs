...
namespace Nghex.Identity.Services
{
        ...
        ) : IAuthManagementService
    {
        ...
        {
            if (!ModelHelper.IsValidId(accountId)) return false;

            var account = await _accountRepository.GetByIdAsync(accountId);
            if (account == null || !account.IsActive || account.IsDeleted) return false;
        ...
        public async Task<IEnumerable<RoleResponse>> GetRolesOfAccountAsync(long accountId)
        {
            var account = await _accountRepository.GetByIdAsync(accountId);
            if (account == null || !account.IsActive || account.IsDeleted) return [];
        ...
        public async Task<IEnumerable<PermissionResponse>> GetPermissionsOfAccountAsync(long accountId)
        {
            var account = await _accountRepository.GetByIdAsync(accountId);
            if (account == null || !account.IsActive || account.IsDeleted) return [];
            ...
            var allPermissions = new List<PermissionResponse>();
        ...
        public async Task<IEnumerable<PermissionResponse>> GetPermissionsOfRoleAsync(long roleId)
        {
            var role = await _roleRepository.GetByIdAsync(roleId);
            if (role == null || !role.IsActive || role.IsDeleted) return [];
        ...
        {
            if (!ModelHelper.IsValidId(roleId)) return false;

            var role = await _roleRepository.GetByIdAsync(roleId);
            if (role == null || !role.IsActive || role.IsDeleted) return false;
        ...
        public async Task<bool> SetPermissionsOnMenuAsync(string menuKey, IEnumerable<string>? permissionCodes)
        {
            ...
            if (menuEntity == null || !menuEntity.IsActive) return false;
        ...
        {
            if (string.IsNullOrWhiteSpace(menuKey) || permissionCodes == null || !permissionCodes.Any()) return false;
        ...
        public async Task<IEnumerable<PermissionWithAssignStatus>> GetPermissionCandidatesOfMenuAsync(string menuKey)
        {
            if (string.IsNullOrWhiteSpace(menuKey)) return [];
            ...
            return candidates.Select(c => new PermissionWithAssignStatus
            {
                Id = c.Id,
                Code = c.Code,
                Name = c.Name,
